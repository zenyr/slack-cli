...
- keep git state clean for its own finalized changes
...
1. Create `result.md` at worktree root with:
   - implemented changes
...
1. Check `todo.md` presence.
2. If present, continue polling.
3. If deleted, treat as candidate completion.
4. Read `result.md`.
5. Delegate verification of claimed outcome to `haiku` or `spark`.
6. If verification passes, accept and continue iteration integration.
7. If verification fails, write updated `todo.md` with precise gap-fix instructions and re-enter polling.
