...
const defaultDeps: ReactionsAddHandlerDeps = {
...
const mapSlackClientError = (error: unknown): CliResult => {
  if (!isSlackClientError(error)) {
    return createError(
      "INTERNAL_ERROR",
      "Unexpected runtime failure for reactions.add.",
...
export const createReactionsAddHandler = (depsOverrides: Partial<ReactionsAddHandlerDeps> = {}) => {
  const deps: ReactionsAddHandlerDeps = {
    ...defaultDeps,
    ...depsOverrides,
  };
  ...
  return async (request: CommandRequest): Promise<CliResult> => {
    ...
    try {
      ...
      return {
        ok: true,
        command: COMMAND_ID,
        message: `Reaction :${data.name}: added to ${data.channel}.`,
        data: {
          channel: data.channel,
          timestamp: data.ts,
          name: data.name,
        },
        textLines: [`Added :${data.name}: to ${data.channel} at ${data.ts}.`],
      };
    } catch (error) {
...
export const reactionsAddHandler = createReactionsAddHandler();
