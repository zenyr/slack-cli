...
const defaultDeps: ChannelsJoinHandlerDeps = {
...
const mapSlackClientError = (error: unknown): CliResult => {
  if (!isSlackClientError(error)) {
    return createError(
      "INTERNAL_ERROR",
      "Unexpected runtime failure for channels.join.",
...
export const createChannelsJoinHandler = (depsOverrides: Partial<ChannelsJoinHandlerDeps> = {}) => {
  const deps: ChannelsJoinHandlerDeps = {
    ...defaultDeps,
    ...depsOverrides,
  };
  ...
  return async (request: CommandRequest): Promise<CliResult> => {
    ...
    try {
      ...
      return {
        ...
        command: COMMAND_ID,
        data: { channel },
        textLines: [`Joined channel #${channel.name} (${channel.id}).`],
      };
    } catch (error) {
...
export const channelsJoinHandler = createChannelsJoinHandler();
